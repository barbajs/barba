...
export default {
  ...
  do(hook, ...args) {
    ...
    this.registered[hook].forEach(hook => hook.fn.apply(hook.ctx, args));
  },
   ...
   */
  help() {
    console.log(`Available hooks: ${this.available}`);
    console.log(`Registered hooks: ${map(this.registered, hook => hook.name)}`);
  },
};
